...
class QuestionController {
	...
	/**
	 * Called when a user add a positive vote for the question
	 * @return
	 */
	def voteUp () {
		def questionVoted = Question.get(params.idquestion)
		def memberWhoVotes = session.user
		
		/* For each vote */
		boolean haveAlreadyVoted = false
		for (Vote v : questionVoted.votes) {
		...
		if (!haveAlreadyVoted) {
			questionVoted.totalVote ++
			questionVoted.save()
			def Vote v = new Vote (votedPost:questionVoted, voter: memberWhoVotes)
			if (v.validate()) {
				v.save()
			}
		}
			
		render questionVoted.totalVote
	}
	
	/**
	 * Called when a user add a negative vote for the question
	 * @return
	 */
	def voteDown () {
